...
class StringFilters extends FilterCollection {
    ...
    static function post_image_url($string){
    	return Util::createImageUrl($string).$string;
    }
    
    static function getMessage($messageCode){
    	return Message::getMessage($messageCode);
    }
    
	static function post_image_size_width($string){
 
    	$imageSize = getimagesize(Util::createImagePath($string).$string);
    	if ( isset($imageSize[0])){
    		return $imageSize[0];
    	}
    }
    
    static function post_image_size_height($string){
    $imageSize = getimagesize(Util::createImagePath($string).$string);
