...
FMS.UserLocation = function(localUser){
	...
	thisUserLocation.getUserLocation = function(){
		$.ajax("/api/getLocation.php",{
			...
			if ( thisUserLocation.marker == null ){
				...
				google.maps.event.addListener(thisUserLocation.marker, 'click', function() {
					FMS.mapsController.map.setZoom(15);
					FMS.mapsController.map.setCenter(thisUserLocation.marker.getPosition());
				  });
			}
			else{
	...
	thisUserLocation.startLocating = function(){
